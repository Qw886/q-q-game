...
运行 `Main.scene` 后，首先看到标题“Qwq连线”和普通、中等、困难三个模式按钮。点击“普通模式”后进入游戏界面，顶部显示“模式：普通、剩余：80、时间：20”，中间显示 10 行 x 8 列共 80 张占位麻将，底部显示“返回菜单”按钮。点击“返回菜单”后回到模式选择页面。
...
6. 点击相邻的相同麻将，确认连接线逐步绘制、两张牌隐藏、剩余数量减少 2、时间恢复 20。
7. 点击不可连接的相同麻将，确认出现失败反馈且不消除。
8. 等待倒计时到 0，确认显示“时间结束”结算界面。
9. 点击“重新开始”，确认棋盘、剩余数量和时间恢复。
...
1. 进入普通模式，确认 80 张麻将、20 秒倒计时。
2. 进入中等模式，确认 120 张麻将、14 秒倒计时。
3. 进入困难模式，确认 192 张麻将、9 秒倒计时。
4. 分别验证消除、连线、倒计时重置、超时失败、死局失败、重新开始和返回菜单。
5. 在三种模式之间反复切换，确认不会残留旧倒计时、旧棋盘或重复事件。
6. 切换 720x1280 竖屏和 1280x720 横屏预览，确认 HUD、棋盘和底部按钮不重叠。
## 麻将牌面图片资源

本项目运行时会自动从 `assets/resources/` 加载麻将图片，不需要手工把图片拖到 Inspector。

资源放置目录：

1. 牌底图片放在 `assets/resources/tiles/backgrounds/tile_bg.png`。
2. 牌面图片放在 `assets/resources/tiles/faces/`。
3. 牌面文件名使用 `wan_1.png`、`dot_1.png`、`bam_1.png`、`east.png`、`red.png` 等约定名称。

图片建议：

1. 推荐尺寸为 `256x320`，或接近 3:4 的竖版比例。
2. 使用透明背景 PNG。
3. 牌面图只包含图案本身，牌底由 `tile_bg.png` 统一提供。

导入和验证：

1. 将 PNG 放入对应目录后，等待 Cocos Creator 资源刷新。
2. 在 Assets 面板中选中图片，确认可生成 `SpriteFrame`。
3. 点击预览运行游戏。
4. 如果图片缺失，麻将会自动显示中文文字兜底，游戏仍应正常运行。
5. 替换图片后重新预览即可，不需要修改场景或重新挂载组件。
